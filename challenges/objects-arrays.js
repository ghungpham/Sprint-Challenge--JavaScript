...
const universities = [];
for (let i = 0; i < graduates.length; i++){universities.push(graduates[i].university);}


// Once you have the new array created, sort the universities alphabetically and log the result. */
console.log(JSON.stringify(universities.sort()))
...
const contactInfo = [];
for (let i = 0; i <graduates.length; i++){contactInfo.push(graduates[i].first_name + " " + graduates[i].email)}
...
const lowerCase = zooAnimals.map(elements => elements.animal_name.toLowerCase());
...
*/
const largerPopulation = zooAnimals.filter(elements => elements.population < 5);
...
*/
const populationTotal = zooAnimals.reduce((accumulator, elements)  => accumulator + elements.population, 0);
